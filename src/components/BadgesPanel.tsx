              ...
              {badges.map((b:any, i:number)=> (
                <li key={i} className="flex items-center gap-3">
                  <div className="w-12 h-12 flex-shrink-0 bg-sky-50 rounded-full flex items-center justify-center overflow-hidden">
                    <img src={`/assets/badges/${b.id}.svg`} alt={b.name} className="w-10 h-10" onError={(e:any)=>{ e.currentTarget.onerror=null; e.currentTarget.style.display='none'; }} />
                    <div className="text-2xl" style={{display: 'none'}}>{b.emoji || '🏅'}</div>
                  </div>
                  <div>
